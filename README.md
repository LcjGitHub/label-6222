...
- 设计师 (`designer`)
- 链接 (`link`)
- 字体分类标签 (`tags`) — 多对多关联，预设五类：`无衬线`、`衬线`、`手写`、`等宽`、`展示体`
  - 创建/更新接口通过 `tag_ids: number[]` 字段绑定
  - 查询接口返回 `tags: { id, name }[]` 数组
...
|------|------|------|
| GET | `/api/tags` | 标签列表 |
| GET | `/api/issues` | 期号列表（含 tags 数组） |
| GET | `/api/issues/:id` | 期号详情（含 tags 数组） |
| POST | `/api/issues` | 新建（可带 tag_ids 绑定标签） |
| PUT | `/api/issues/:id` | 更新（可带 tag_ids 绑定标签） |
